// RUN: circt-opt %s --arc-lower-to-llvm | FileCheck %s
...
// CHECK-LABEL: llvm.func @ComplexArc(%arg0: i4, %arg1: i4) -> i4 {
arc.define @ComplexArc(%arg0: i4, %arg1: i4) -> i4 {
  %0 = comb.add %arg0, %arg1 : i4
  // CHECK-NEXT: [[TMP:%.+]] = llvm.add %arg0, %arg1 : i4
  %1 = arc.state @ComplexArc(%0, %0) lat 0 : (i4, i4) -> i4
  // CHECK-NEXT: [[TMP2:%.+]] = llvm.call @ComplexArc([[TMP]], [[TMP]]) : (i4, i4) -> i4
  arc.output %1 : i4
  // CHECK-NEXT: llvm.return [[TMP2]] : i4
}
// CHECK-NEXT: }
...
func.func @StateAllocation(%arg0: !arc.storage<10>) {
  ...
  // CHECK-NEXT: [[OFFSET:%.+]] = llvm.mlir.constant(3 :
  // CHECK-NEXT: [[PTR:%.+]] = llvm.getelementptr %arg0[[[OFFSET]]]
  ...
  // CHECK-NEXT: [[OFFSET:%.+]] = llvm.mlir.constant(7 :
  // CHECK-NEXT: [[PTR:%.+]] = llvm.getelementptr %arg0[[[OFFSET]]]
  return
  // CHECK-NEXT: llvm.return
}
...
func.func @StateUpdates(%arg0: !arc.storage<1>) {
  ...
  // CHECK-NEXT: [[OFFSET:%.+]] = llvm.mlir.constant(0 :
  // CHECK-NEXT: [[RAW_PTR:%.+]] = llvm.getelementptr %arg0[[[OFFSET]]]
  // CHECK-NEXT: [[PTR:%.+]] = llvm.bitcast [[RAW_PTR]] : !llvm.ptr<i8> to !llvm.ptr<i1>

  %1 = arc.state_read %0 : <i1>
  // CHECK-NEXT: [[TMP:%.+]] = llvm.load [[PTR]]
  arc.state_write %0 = %1 : <i1>
  // CHECK-NEXT: llvm.store [[TMP]], [[PTR]]
  return
  // CHECK-NEXT: llvm.return
}
// CHECK-NEXT: }
