...
firrtl.circuit "Foo" {
  ...
  firrtl.module @Statements(in %ui1: !firrtl.uint<1>, in %someAddr: !firrtl.uint<8>, in %someClock: !firrtl.clock, in %someReset: !firrtl.reset, out %someOut: !firrtl.uint<1>) {
    ...
    %k0 = firrtl.node %0 : !firrtl.const.uint<19>
    %k1 = firrtl.node %1 : !firrtl.const.sint<19>
    %k2 = firrtl.node %2 : !firrtl.const.uint
    %k3 = firrtl.node %3 : !firrtl.const.sint
    ...
    %x = firrtl.node %2 : !firrtl.const.uint
    %y = firrtl.node %2 : !firrtl.const.uint
    ...
    %addPrimOp_tmp = firrtl.add %x, %y : (!firrtl.const.uint, !firrtl.const.uint) -> !firrtl.const.uint
    %subPrimOp_tmp = firrtl.sub %x, %y : (!firrtl.const.uint, !firrtl.const.uint) -> !firrtl.const.uint
    %mulPrimOp_tmp = firrtl.mul %x, %y : (!firrtl.const.uint, !firrtl.const.uint) -> !firrtl.const.uint
    %divPrimOp_tmp = firrtl.div %x, %y : (!firrtl.const.uint, !firrtl.const.uint) -> !firrtl.const.uint
    %remPrimOp_tmp = firrtl.rem %x, %y : (!firrtl.const.uint, !firrtl.const.uint) -> !firrtl.const.uint
    %andPrimOp_tmp = firrtl.and %x, %y : (!firrtl.const.uint, !firrtl.const.uint) -> !firrtl.const.uint
    %orPrimOp_tmp = firrtl.or %x, %y : (!firrtl.const.uint, !firrtl.const.uint) -> !firrtl.const.uint
    %xorPrimOp_tmp = firrtl.xor %x, %y : (!firrtl.const.uint, !firrtl.const.uint) -> !firrtl.const.uint
    %leqPrimOp_tmp = firrtl.leq %x, %y : (!firrtl.const.uint, !firrtl.const.uint) -> !firrtl.const.uint<1>
    %ltPrimOp_tmp = firrtl.lt %x, %y : (!firrtl.const.uint, !firrtl.const.uint) -> !firrtl.const.uint<1>
    %geqPrimOp_tmp = firrtl.geq %x, %y : (!firrtl.const.uint, !firrtl.const.uint) -> !firrtl.const.uint<1>
    %gtPrimOp_tmp = firrtl.gt %x, %y : (!firrtl.const.uint, !firrtl.const.uint) -> !firrtl.const.uint<1>
    %eqPrimOp_tmp = firrtl.eq %x, %y : (!firrtl.const.uint, !firrtl.const.uint) -> !firrtl.const.uint<1>
    %neqPrimOp_tmp = firrtl.neq %x, %y : (!firrtl.const.uint, !firrtl.const.uint) -> !firrtl.const.uint<1>
    %catPrimOp_tmp = firrtl.cat %x, %y : (!firrtl.const.uint, !firrtl.const.uint) -> !firrtl.const.uint
    %dShlPrimOp_tmp = firrtl.dshl %x, %y : (!firrtl.const.uint, !firrtl.const.uint) -> !firrtl.const.uint
    %dShlwPrimOp_tmp = firrtl.dshlw %x, %y : (!firrtl.const.uint, !firrtl.const.uint) -> !firrtl.const.uint
    %dShrPrimOp_tmp = firrtl.dshr %x, %y : (!firrtl.const.uint, !firrtl.const.uint) -> !firrtl.const.uint
    %addPrimOp = firrtl.node %addPrimOp_tmp : !firrtl.const.uint
    %subPrimOp = firrtl.node %subPrimOp_tmp : !firrtl.const.uint
    %mulPrimOp = firrtl.node %mulPrimOp_tmp : !firrtl.const.uint
    %divPrimOp = firrtl.node %divPrimOp_tmp : !firrtl.const.uint
    %remPrimOp = firrtl.node %remPrimOp_tmp : !firrtl.const.uint
    %andPrimOp = firrtl.node %andPrimOp_tmp : !firrtl.const.uint
    %orPrimOp = firrtl.node %orPrimOp_tmp : !firrtl.const.uint
    %xorPrimOp = firrtl.node %xorPrimOp_tmp : !firrtl.const.uint
    %leqPrimOp = firrtl.node %leqPrimOp_tmp : !firrtl.const.uint<1>
    %ltPrimOp = firrtl.node %ltPrimOp_tmp : !firrtl.const.uint<1>
    %geqPrimOp = firrtl.node %geqPrimOp_tmp : !firrtl.const.uint<1>
    %gtPrimOp = firrtl.node %gtPrimOp_tmp : !firrtl.const.uint<1>
    %eqPrimOp = firrtl.node %eqPrimOp_tmp : !firrtl.const.uint<1>
    %neqPrimOp = firrtl.node %neqPrimOp_tmp : !firrtl.const.uint<1>
    %catPrimOp = firrtl.node %catPrimOp_tmp : !firrtl.const.uint
    %dShlPrimOp = firrtl.node %dShlPrimOp_tmp : !firrtl.const.uint
    %dShlwPrimOp = firrtl.node %dShlwPrimOp_tmp : !firrtl.const.uint
    %dShrPrimOp = firrtl.node %dShrPrimOp_tmp : !firrtl.const.uint
    ...
    %asSIntPrimOp_tmp = firrtl.asSInt %x : (!firrtl.const.uint) -> !firrtl.const.sint
    %asUIntPrimOp_tmp = firrtl.asUInt %x : (!firrtl.const.uint) -> !firrtl.const.uint
    %asAsyncResetPrimOp_tmp = firrtl.asAsyncReset %x : (!firrtl.const.uint) -> !firrtl.const.asyncreset
    %asClockPrimOp_tmp = firrtl.asClock %x : (!firrtl.const.uint) -> !firrtl.const.clock
    %cvtPrimOp_tmp = firrtl.cvt %x : (!firrtl.const.uint) -> !firrtl.const.sint
    %negPrimOp_tmp = firrtl.neg %x : (!firrtl.const.uint) -> !firrtl.const.sint
    %notPrimOp_tmp = firrtl.not %x : (!firrtl.const.uint) -> !firrtl.const.uint
    %andRPrimOp_tmp = firrtl.andr %x : (!firrtl.const.uint) -> !firrtl.const.uint<1>
    %orRPrimOp_tmp = firrtl.orr %x : (!firrtl.const.uint) -> !firrtl.const.uint<1>
    %xorRPrimOp_tmp = firrtl.xorr %x : (!firrtl.const.uint) -> !firrtl.const.uint<1>
    %asSIntPrimOp = firrtl.node %asSIntPrimOp_tmp : !firrtl.const.sint
    %asUIntPrimOp = firrtl.node %asUIntPrimOp_tmp : !firrtl.const.uint
    %asAsyncResetPrimOp = firrtl.node %asAsyncResetPrimOp_tmp : !firrtl.const.asyncreset
    %asClockPrimOp = firrtl.node %asClockPrimOp_tmp : !firrtl.const.clock
    %cvtPrimOp = firrtl.node %cvtPrimOp_tmp : !firrtl.const.sint
    %negPrimOp = firrtl.node %negPrimOp_tmp : !firrtl.const.sint
    %notPrimOp = firrtl.node %notPrimOp_tmp : !firrtl.const.uint
    %andRPrimOp = firrtl.node %andRPrimOp_tmp : !firrtl.const.uint<1>
    %orRPrimOp = firrtl.node %orRPrimOp_tmp : !firrtl.const.uint<1>
    %xorRPrimOp = firrtl.node %xorRPrimOp_tmp : !firrtl.const.uint<1>
    ...
    %bitsPrimOp_tmp = firrtl.bits %x 4 to 2 : (!firrtl.const.uint) -> !firrtl.const.uint<3>
    %headPrimOp_tmp = firrtl.head %x, 4 : (!firrtl.const.uint) -> !firrtl.const.uint<4>
    %tailPrimOp_tmp = firrtl.tail %x, 4 : (!firrtl.const.uint) -> !firrtl.const.uint
    %padPrimOp_tmp = firrtl.pad %x, 16 : (!firrtl.const.uint) -> !firrtl.const.uint
    %muxPrimOp_tmp = firrtl.mux(%ui1, %x, %y) : (!firrtl.uint<1>, !firrtl.const.uint, !firrtl.const.uint) -> !firrtl.uint
    %shlPrimOp_tmp = firrtl.shl %x, 4 : (!firrtl.const.uint) -> !firrtl.const.uint
    %shrPrimOp_tmp = firrtl.shr %x, 4 : (!firrtl.const.uint) -> !firrtl.const.uint
    %bitsPrimOp = firrtl.node %bitsPrimOp_tmp : !firrtl.const.uint<3>
    %headPrimOp = firrtl.node %headPrimOp_tmp : !firrtl.const.uint<4>
    %tailPrimOp = firrtl.node %tailPrimOp_tmp : !firrtl.const.uint
    %padPrimOp = firrtl.node %padPrimOp_tmp : !firrtl.const.uint
    %muxPrimOp = firrtl.node %muxPrimOp_tmp : !firrtl.uint
    %shlPrimOp = firrtl.node %shlPrimOp_tmp : !firrtl.const.uint
    %shrPrimOp = firrtl.node %shrPrimOp_tmp : !firrtl.const.uint
