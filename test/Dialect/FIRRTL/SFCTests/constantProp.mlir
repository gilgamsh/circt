...
firrtl.circuit "ConstInput"   {
  firrtl.module @ConstInput(in %x: !firrtl.uint<1>, out %z: !firrtl.uint<1>) {
    ...
    firrtl.connect %c_in1, %c1_ui1 : !firrtl.uint<1>, !firrtl.const.uint<1>
...
firrtl.circuit "InstanceInput"   {
  ...
  firrtl.module private @Child1(out %out: !firrtl.uint<1>) {
    ...
    firrtl.connect %b0_in, %c1_ui : !firrtl.uint<1>, !firrtl.const.uint
  ...
  firrtl.module @InstanceInput(in %x: !firrtl.uint<1>, out %z: !firrtl.uint<1>) {
    ...
    firrtl.connect %b1_in, %c1_ui : !firrtl.uint<1>, !firrtl.const.uint
    ...
    // CHECK: %[[C0:.+]] = firrtl.constant 1 : !firrtl.const.uint<1>
...
firrtl.circuit "acrossWire"   {
  ...
  firrtl.module @acrossWire(in %x: !firrtl.uint<1>, out %y: !firrtl.uint<1>) {
    ...
    %0 = firrtl.mux(%x, %c0_ui1, %c0_ui1) : (!firrtl.uint<1>, !firrtl.const.uint<1>, !firrtl.const.uint<1>) -> !firrtl.uint<1>
    firrtl.connect %_z, %0 : !firrtl.uint<1>, !firrtl.uint<1>
    // CHECK: %[[C2:.+]] = firrtl.constant 0 : !firrtl.const.uint<1>
...
firrtl.circuit "constOutput"   {
  firrtl.module private @constOutChild(out %out: !firrtl.uint<1>) {
    %c0_ui1 = firrtl.constant 0 : !firrtl.const.uint<1>
    firrtl.connect %out, %c0_ui1 : !firrtl.uint<1>, !firrtl.const.uint<1>
  }
  firrtl.module @constOutput(in %x: !firrtl.uint<1>, out %z: !firrtl.uint<1>) {
    ...
    // CHECK: %[[C3_0:.+]] = firrtl.constant 0 : !firrtl.const.uint<1>
    // CHECK: %[[C3:.+]] = firrtl.constant 0 : !firrtl.const.uint<1>
...
firrtl.circuit "optiMux"   {
  ...
  firrtl.module @optiMux(out %z: !firrtl.uint<4>) {
    ...
    %0 = firrtl.mux(%c1_ui, %c0_ui2, %c0_ui4) : (!firrtl.const.uint, !firrtl.const.uint<2>, !firrtl.const.uint<4>) -> !firrtl.const.uint<4>
...
firrtl.circuit "divFold"   {
  ...
  firrtl.module @divFold(in %a: !firrtl.uint<8>, out %b: !firrtl.uint<8>) {
    ...
    // CHECK: %[[C5:.+]] = firrtl.constant 1 : !firrtl.const.uint<8>
    // CHECK: firrtl.strictconnect %b, %[[C5]] : !firrtl.uint<8>, !firrtl.const.uint<8>
  }
}
...
firrtl.circuit "padConstWire"   {
  ...
  firrtl.module @padConstWire(out %z: !firrtl.uint<16>) {
    ...
    firrtl.connect %_w_a, %c3_ui2 : !firrtl.uint<8>, !firrtl.const.uint<2>
    firrtl.connect %_w_b, %c3_ui2 : !firrtl.uint<8>, !firrtl.const.uint<2>
    %0 = firrtl.cat %_w_a, %_w_b : (!firrtl.uint<8>, !firrtl.uint<8>) -> !firrtl.uint<16>
    firrtl.connect %z, %0 : !firrtl.uint<16>, !firrtl.uint<16>
    // CHECK: %[[C6:.+]] = firrtl.constant 771 : !firrtl.const.uint<16>
    // CHECK-NEXT: firrtl.strictconnect %z, %[[C6]] : !firrtl.uint<16>, !firrtl.const.uint<16>
  }
}
...
firrtl.circuit "padConstReg"   {
  ...
  firrtl.module @padConstReg(in %clock: !firrtl.clock, out %z: !firrtl.uint<16>) {
    ...
    %c3_ui2 = firrtl.constant 3 : !firrtl.const.uint<2>
    firrtl.connect %r_a, %c3_ui2 : !firrtl.uint<8>, !firrtl.const.uint<2>
    firrtl.connect %r_b, %c3_ui2 : !firrtl.uint<8>, !firrtl.const.uint<2>
...
firrtl.circuit "padConstOut"   {
  firrtl.module private @padConstOutChild(out %x: !firrtl.uint<8>) {
    %c3_ui2 = firrtl.constant 3 : !firrtl.const.uint<2>
    firrtl.connect %x, %c3_ui2 : !firrtl.uint<8>, !firrtl.const.uint<2>
  }
  ...
  firrtl.module @padConstOut(out %z: !firrtl.uint<16>) {
    ...
    %c3_ui2 = firrtl.constant 3 : !firrtl.const.uint<2>
    %0 = firrtl.cat %c3_ui2, %c_x : (!firrtl.const.uint<2>, !firrtl.uint<8>) -> !firrtl.uint<10>
    // CHECK: %[[C8:.+]] = firrtl.constant 771 : !firrtl.const.uint<16>
    // CHECK: firrtl.strictconnect %z, %[[C8]] : !firrtl.uint<16>, !firrtl.const.uint<16>
...
firrtl.circuit "padConstIn"   {
  ...
  firrtl.module private @padConstInChild(in %x: !firrtl.uint<8>, out %y: !firrtl.uint<16>) {
    %c3_ui2 = firrtl.constant 3 : !firrtl.const.uint<2>
    %0 = firrtl.cat %c3_ui2, %x : (!firrtl.const.uint<2>, !firrtl.uint<8>) -> !firrtl.uint<10>
    // CHECK: %[[C9:.+]] = firrtl.constant 771 : !firrtl.const.uint<16>
    // CHECK: firrtl.strictconnect %y, %[[C9]] : !firrtl.uint<16>, !firrtl.const.uint<16>
  ...
  firrtl.module @padConstIn(out %z: !firrtl.uint<16>) {
    ...
    %c3_ui2 = firrtl.constant 3 : !firrtl.const.uint<2>
    firrtl.connect %c_x, %c3_ui2 : !firrtl.uint<8>, !firrtl.const.uint<2>
    firrtl.connect %z, %c_y : !firrtl.uint<16>, !firrtl.uint<16>
    // CHECK: %[[C10:.+]] = firrtl.constant 771 : !firrtl.const.uint<16>
    // CHECK: firrtl.strictconnect %z, %[[C10]] : !firrtl.uint<16>, !firrtl.const.uint<16>
  }
}
...
firrtl.circuit "constResetReg"   {
  ...
  firrtl.module @constResetReg(in %clock: !firrtl.clock, in %reset: !firrtl.uint<1>, out %z: !firrtl.uint<8>) {
    ...
    // CHECK: %[[C11:.+]] = firrtl.constant 11 : !firrtl.const.uint<8>
    // CHECK: firrtl.strictconnect %z, %[[C11]] : !firrtl.uint<8>, !firrtl.const.uint<8>
  }
}
...
firrtl.circuit "asyncReset"   {
  ...
  firrtl.module @asyncReset(in %clock: !firrtl.clock, in %reset: !firrtl.asyncreset, in %en: !firrtl.uint<1>, out %z: !firrtl.uint<8>) {
    %c11_ui4 = firrtl.constant 11 : !firrtl.const.uint<4>
    %r = firrtl.regreset %clock, %reset, %c11_ui4  : !firrtl.clock, !firrtl.asyncreset, !firrtl.const.uint<4>, !firrtl.uint<8>
    %c0_ui4 = firrtl.constant 0 : !firrtl.const.uint<4>
    %0 = firrtl.mux(%en, %c0_ui4, %r) : (!firrtl.uint<1>, !firrtl.const.uint<4>, !firrtl.uint<8>) -> !firrtl.uint<8>
...
firrtl.circuit "constReg2"   {
  ...
  firrtl.module @constReg2(in %clock: !firrtl.clock, in %reset: !firrtl.uint<1>, out %z: !firrtl.sint<8>) {
    ...
    %c-5_si4 = firrtl.constant -5 : !firrtl.const.sint<4>
    firrtl.connect %r, %c-5_si4 : !firrtl.sint<8>, !firrtl.const.sint<4>
    firrtl.connect %z, %r : !firrtl.sint<8>, !firrtl.sint<8>
    // CHECK: %[[C12:.+]] = firrtl.constant -5 : !firrtl.const.sint<8>
    // CHECK: firrtl.strictconnect %z, %[[C12]] : !firrtl.sint<8>, !firrtl.const.sint<8>
  }
}
...
firrtl.circuit "regSameConstReset"   {
  ...
  firrtl.module @regSameConstReset(in %clock: !firrtl.clock, in %reset: !firrtl.uint<1>, out %z: !firrtl.uint<8>) {
    %c11_ui4 = firrtl.constant 11 : !firrtl.const.uint<4>
    %r = firrtl.regreset %clock, %reset, %c11_ui4  : !firrtl.clock, !firrtl.uint<1>, !firrtl.const.uint<4>, !firrtl.uint<8>
    firrtl.connect %r, %c11_ui4 : !firrtl.uint<8>, !firrtl.const.uint<4>
    firrtl.connect %z, %r : !firrtl.uint<8>, !firrtl.uint<8>
    // CHECK: %[[C13:.+]] = firrtl.constant 11 : !firrtl.const.uint<8>
    // CHECK: firrtl.strictconnect %z, %[[C13]] : !firrtl.uint<8>, !firrtl.const.uint<8>
  }
}
...
firrtl.circuit "SignTester"   {
  ...
  firrtl.module @SignTester(out %ref: !firrtl.sint<3>) {
    %c0_ui1 = firrtl.constant 0 : !firrtl.const.uint<1>
    %c0_si3 = firrtl.constant 0 : !firrtl.const.sint<3>
    %c3_ui2 = firrtl.constant 3 : !firrtl.const.uint<2>
    %0 = firrtl.neg %c3_ui2 : (!firrtl.const.uint<2>) -> !firrtl.const.sint<3>
    %1 = firrtl.mux(%c0_ui1, %c0_si3, %0) : (!firrtl.const.uint<1>, !firrtl.const.sint<3>, !firrtl.const.sint<3>) -> !firrtl.const.sint<3>
    firrtl.connect %ref, %1 : !firrtl.sint<3>, !firrtl.const.sint<3>
    // CHECK:  %[[C14:.+]] = firrtl.constant -3 : !firrtl.const.sint<3>
    // CHECK:  firrtl.strictconnect %ref, %[[C14]] : !firrtl.sint<3>, !firrtl.const.sint<3>
  }
}
...
firrtl.circuit "AddTester"   {
  ...
  firrtl.module @AddTester(out %ref: !firrtl.sint<2>) {
    ...
    %0 = firrtl.add %c-1_si1, %c-1_si1 : (!firrtl.const.sint<1>, !firrtl.const.sint<1>) -> !firrtl.const.sint<2>
    firrtl.connect %ref, %0 : !firrtl.sint<2>, !firrtl.const.sint<2>
    // CHECK:  %[[C15:.+]] = firrtl.constant -2 : !firrtl.const.sint<2>
...
firrtl.circuit "ConstPropReductionTester"   {
  ...
  firrtl.module @ConstPropReductionTester(out %out1: !firrtl.uint<1>, out %out2: !firrtl.uint<1>, out %out3: !firrtl.uint<1>) {
    ...
    %0 = firrtl.xorr %c-1_si2 : (!firrtl.const.sint<2>) -> !firrtl.const.uint<1>
    firrtl.connect %out1, %0 : !firrtl.uint<1>, !firrtl.const.uint<1>
    %1 = firrtl.andr %c-1_si2 : (!firrtl.const.sint<2>) -> !firrtl.const.uint<1>
    firrtl.connect %out2, %1 : !firrtl.uint<1>, !firrtl.const.uint<1>
    %2 = firrtl.orr %c-1_si2 : (!firrtl.const.sint<2>) -> !firrtl.const.uint<1>
    firrtl.connect %out3, %2 : !firrtl.uint<1>, !firrtl.const.uint<1>
...
firrtl.circuit "TailTester"   {
  ...
  firrtl.module @TailTester(out %out: !firrtl.uint<1>) {
    %c0_ui1 = firrtl.constant 0 : !firrtl.const.uint<1>
    %c23_ui5 = firrtl.constant 23 : !firrtl.const.uint<5>
    %0 = firrtl.add %c0_ui1, %c23_ui5 : (!firrtl.const.uint<1>, !firrtl.const.uint<5>) -> !firrtl.const.uint<6>
    %_temp = firrtl.node droppable_name %0  : !firrtl.const.uint<6>
    %1 = firrtl.head %_temp, 3 : (!firrtl.const.uint<6>) -> !firrtl.const.uint<3>
    %_head_temp = firrtl.node droppable_name %1  : !firrtl.const.uint<3>
    %2 = firrtl.tail %_head_temp, 2 : (!firrtl.const.uint<3>) -> !firrtl.const.uint<1>
    firrtl.connect %out, %2 : !firrtl.uint<1>, !firrtl.const.uint<1>
...
firrtl.circuit "TailTester2"   {
  ...
  firrtl.module @TailTester2(out %out: !firrtl.uint<1>) {
    ...
    %0 = firrtl.add %c0_ui1, %c23_ui5 : (!firrtl.const.uint<1>, !firrtl.const.uint<5>) -> !firrtl.const.uint<6>
    %_temp = firrtl.node droppable_name %0  : !firrtl.const.uint<6>
    %1 = firrtl.tail %_temp, 1 : (!firrtl.const.uint<6>) -> !firrtl.const.uint<5>
    %_tail_temp = firrtl.node droppable_name %1  : !firrtl.const.uint<5>
    %2 = firrtl.tail %_tail_temp, 4 : (!firrtl.const.uint<5>) -> !firrtl.const.uint<1>
    firrtl.connect %out, %2 : !firrtl.uint<1>, !firrtl.const.uint<1>
...
firrtl.circuit "ZeroWidthAdd"   {
  ...
  firrtl.module @ZeroWidthAdd(in %x: !firrtl.uint<0>, out %y: !firrtl.uint<7>) {
    %c0_ui9 = firrtl.constant 0 : !firrtl.const.uint<9>
    %0 = firrtl.add %x, %c0_ui9 : (!firrtl.uint<0>, !firrtl.const.uint<9>) -> !firrtl.uint<10>
