...
private:
  bool checkImpl(DefineOp arcA, DefineOp arcB) {
    ...
    auto outputA = cast<arc::OutputOp>(arcA.getBodyBlock().getTerminator());
    auto outputB = cast<arc::OutputOp>(arcB.getBodyBlock().getTerminator());
    if (outputA.getNumOperands() != outputB.getNumOperands())
      return false;
    for (auto [a, b] :
         llvm::zip(outputA->getOpOperands(), outputB->getOpOperands()))
...
void DedupPass::runOnOperation() {
  ...
  for (unsigned arcIdx = 0, arcEnd = arcHashes.size(); arcIdx != arcEnd;
    ...
    for (unsigned otherIdx = arcIdx + 1; otherIdx != arcEnd; ++otherIdx) {
      ...
      // value pair the the operands are set to, and then store the current
      // arc's operand in the set that corresponds to this value pair. This
      // builds up `operandMappings` to contain sets of op operands in the
      // current arc that can be routed out to the same block argument. If a
      // block argument of the current arc corresponds to multiple different
      // things in the other arc, this ensures that all unique such combinations
      // get grouped in distinct sets such that we can create an appropriate
      // number of new block args.
