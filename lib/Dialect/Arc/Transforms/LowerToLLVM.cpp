...
struct AllocStateOpLowering : public ConversionPattern {
  ...
  matchAndRewrite(Operation *op, ArrayRef<Value> operands,
    ...
    auto offsetAttr = op->getAttr("offset");
    assert(offsetAttr && "cannot lower unallocated state to LLVM");
    Value offset = rewriter.create<LLVM::ConstantOp>(
    ...
    auto ptrType = LLVM::LLVMPointerType::get(
        op->getResult(0).getType().cast<StateType>().getType());
...
LogicalResult LowerToLLVMPass::lowerToMLIR() {
...
LogicalResult LowerToLLVMPass::lowerToLLVM() {
  ...
  populateHWToLLVMConversionPatterns(typeConverter, patterns);
  populateHWToLLVMTypeConversions(typeConverter);
  populateCombToLLVMConversionPatterns(typeConverter, patterns);
  return applyFullConversion(getOperation(), target, std::move(patterns));
}

std::unique_ptr<Pass> arc::createLowerToLLVMPass() {
