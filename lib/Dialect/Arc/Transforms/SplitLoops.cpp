...
struct SplitLoopsPass : public SplitLoopsBase<SplitLoopsPass> {
  ...
  LogicalResult splitArcDefinition(DefineOp defOp, const APInt &resultsToSplit);
...
void SplitLoopsPass::runOnOperation() {
  ...
  for (auto defOp : module.getOps<DefineOp>()) {
    arcNamespace.newName(defOp.getSymName());
    analyzeArcDef(defOp);
  }
...
void SplitLoopsPass::analyzeArcDef(DefineOp defOp) {
  auto &info = defInfo[defOp.getSymNameAttr()];
  info.defOp = defOp;
  ...
  for (auto &op : defOp.getBodyBlock()) {
  ...
  auto outputOp = cast<arc::OutputOp>(defOp.getBodyBlock().getTerminator());
...
void SplitLoopsPass::analyzeModule(HWModuleOp moduleOp) {
  ...
  for (auto &op : *moduleOp.getBodyBlock()) {
    if (auto stateOp = dyn_cast<StateOp>(&op)) {
      defInfo[stateOp.getArcAttr().getAttr()].uses.push_back(stateOp);
      ...
      while (!worklist.empty()) {
        ...
        if (auto stateOp = dyn_cast<StateOp>(op); stateOp && idx == 0) {
          ...
          if (seenResults.countPopulation() > 1)
            loopyResults[stateOp.getArcAttr().getAttr()].insert(seenResults);
        }
        ...
        if (auto stateOp = dyn_cast<StateOp>(op)) {
          auto &info = defInfo[stateOp.getArcAttr().getAttr()];
...
LogicalResult SplitLoopsPass::splitLoopyArc(DefineOp defOp,
  ...
  LLVM_DEBUG({
    llvm::dbgs() << "Splitting arc @" << defOp.getSymName() << " with "
  ...
  splitMasks.clear();
  for (auto &op : llvm::reverse(defOp.getBodyBlock())) {
...
LogicalResult SplitLoopsPass::splitArcDefinition(DefineOp defOp,
  ...
  for (auto *arc : arcs) {
    ...
    arc->defOp = builder.create<DefineOp>(
        defOp.getLoc(),
        builder.getStringAttr(arcNamespace.newName(defOp.getSymName() +
                                                   "_split_" + Twine(arc->id))),
        builder.getFunctionType(inputTypes, outputTypes));
    arc->defOp.getBody().push_back(arc->block.release());
  }

  // Replace uses of the original arc with the split arcs.
  for (auto oldStateOp : defInfo[defOp.getSymNameAttr()].uses) {
    LLVM_DEBUG(llvm::dbgs()
    ...
    for (auto [useOperand, defArg] :
         llvm::zip(oldStateOp.getInputs(), defOp.getArguments()))
    ...
    for (auto *arc : arcs) {
      ...
      LLVM_DEBUG(llvm::dbgs() << "  - Using @" << arc->defOp.getSymName()
                              << (isPrefix ? " (prefix)" : "") << "\n");
      ...
      auto newStateOp = builder.create<StateOp>(
          oldStateOp.getLoc(), arc->defOp,
          isPrefix ? Value{} : oldStateOp.getClock(),
          isPrefix ? Value{} : oldStateOp.getEnable(),
          isPrefix ? 0 : oldStateOp.getLatency(), inputs);
