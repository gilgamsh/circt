...
struct ClockLowering {
  ...
  DenseMap<Value, Value> materializedValues;
...
Value ClockLowering::materializeValue(Value value) {
  if (!shouldMaterialize(value))
    return value;
  if (auto mapped = materializedValues.lookup(value))
    return mapped;
  ...
  while (!worklist.empty()) {
    ...
    if (mappedOperands.size() < op->getNumOperands()) {
      ...
      auto mapped = materializedValues.lookup(operand);
      if (!mapped) {
        if (shouldMaterialize(operand)) {
      ...
      // TODO: Try not to clone operations. Maybe we can drop the use references
      // of the op on the worklist early, such that its dependencies already see
      // the op as moved.
      Operation *newOp = op->cloneWithoutRegions();
      ++stats.matOpsCloned;
      builder.insert(newOp);
      for (auto [oldArg, newArg] :
           llvm::zip(newOp->getOpOperands(), mappedOperands)) {
        assert(newArg);
        oldArg.set(newArg);
      }
      for (auto [oldResult, newResult] :
           llvm::zip(op->getResults(), newOp->getResults()))
        materializedValues.insert({oldResult, newResult});
  ...
  auto mapped = materializedValues.lookup(value);
  assert(mapped);
  return mapped;
}
...
void ModuleLowering::replaceValueWithStateRead(Value value, Value state) {
  SmallDenseMap<Block *, Value> readsByBlock;
  for (auto &use : llvm::make_early_inc_range(value.getUses())) {
    auto *op = use.getOwner();
    auto &read = readsByBlock[op->getBlock()];
    if (!read) {
      OpBuilder builder(op);
      read = builder.create<StateReadOp>(value.getLoc(), state);
    } else {
      // We have no guarantee that we iterate over the uses in the order
      // they appear in the block. Therefore make sure we move the read op
      // before the earliest use.
      auto *readOp = read.getDefiningOp();
      if (op->isBeforeInBlock(readOp))
        readOp->moveBefore(op);
    }
    use.set(read);
  }
}
...
LogicalResult ModuleLowering::lowerState(StateOp stateOp) {
  ...
  // Arcs with latency must be synchronous to a clock.
  if (!stateOp.getClock()) {
  ...
  info.enable = info.clock.getOrCreateAnd(info.enable, stateOp.getEnable(),
                                          stateOp.getLoc());
  ...
  for (unsigned stateIdx = 0; stateIdx < stateOp.getNumResults(); ++stateIdx) {
    auto stateType = StateType::get(
        stateOp.getResult(stateIdx).getType().cast<IntegerType>());
    auto state =
...
LogicalResult ModuleLowering::cleanup() {
  for (auto &op : llvm::make_early_inc_range(*moduleOp.getBodyBlock())) {
    ...
    // Make sure that all uses of the operation are in the module body, not
    // inside any clock trees. The clock trees should have created copies of the
    // operations that they need for computation.
    for (auto *user : op.getUsers()) {
      if (user->getParentOp() == moduleOp)
        continue;
      auto d = op.emitError(
